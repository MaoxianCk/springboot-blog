		...
		try {
			...
			for (int i = 1; i <= 10; i++) {
				articleInfoDaoImpl.insertArticleInfo(
						new ArticleInfo(0, "title " + String.valueOf(i), "summary " + String.valueOf(i), true, i));
			}
			// 插入5条记录 时间为当前时间
			for (int i = 6; i <= 10; i++) {
				articleInfoDaoImpl.insertArticleInfo(
			...
			// 更新id 3 的记录
			ArticleInfo recordArticleInfo = new ArticleInfo(3, "title updated", "summary updated", false, 99);
			...
			_list = articleInfoDaoImpl.selectArticleInfoByTitle("uphvgc");
			printArticleInfoList(_list);
			
			_list = articleInfoDaoImpl.selectArticleInfoBySummaryParts("upfhx");
			printArticleInfoList(_list);
